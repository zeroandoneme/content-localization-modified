...
<template>
  <div>
    ...
    <div v-else-if="noTranslation === false" class="wrapper">
      ...
      <div
        v-if="selected_lang_code !== ''"
        id="event-line-editor"
        class="event-line-editor"
      >
        <b-table
          ref="selectableTable"
          selectable
      ...
      <b-button
        id="saveTerminology"
        v-b-tooltip.hover
        title="Save terminology will open a window where you can create or modify custom terminologies for AWS Translate"
        size="sm"
        class="mb-2"
        @click="showTerminologyConfirmation()"
      >
        <b-icon icon="card-text" color="white"></b-icon>
        Save terminology </b-button
      >&nbsp;
      ...
      <br />
      <b-modal
        ref="delete-terminology-modal"
        ok-title="Confirm"
        ok-variant="danger"
        title="Delete Terminology?"
        @ok="
          deleteTerminologyRequest(
            (customTerminologySelected = customTerminologySelected)
          )
        "
      >
        <p>
          Are you sure you want to permanently delete the custom terminology
          <b>{{ customTerminologySelected }}</b
          >?
        </p>
      </b-modal>
      <b-modal
        ...
        :ok-disabled="
          validTerminologyName === false ||
          customTerminologyCreateNew === '' ||
          customTerminologyUnion.length === 0 ||
          validCSV === false
        "
        ok-title="Save"
        @ok="saveTerminology()"
      ...
      >
        ...
        <div v-if="customTerminologySelected.length > 0">
          <div v-if="customTerminologySelected.length === 1">
            ...
            <b-button
              ...
              size="sm"
              title="Delete selected terminology"
              variant="danger"
          ...
          <div v-if="customTerminologySelected.length > 1">
            ...
            <b-button
              v-b-tooltip.hover.right
              size="sm"
              title="Delete selected terminologies"
              variant="danger"
              @click="deleteTerminology"
            >
              Delete
        ...
        <div
          v-if="
            customTerminologyUnsaved.length !== 0 ||
            customTerminologySaved.length !== 0
          "
        >
          ...
          <b-form-input
            v-model="customTerminologyCreateNew"
            size="sm"
            :state="validTerminologyName ? null : false"
          ></b-form-input>
        </div>
        <div v-if="customTerminologyUnsaved.length !== 0">
          New terminology content (click to edit):
          <div
            v-if="customTerminologySelected.length > 0"
            class="text-info"
            style="font-size: 80%"
          >
            Rows shown in blue were loaded from
            <b>{{ customTerminologySelected }}.</b>
          </div>
        </div>
        <b-table
          ...
          show-empty
        >
          <template #default> No data </template>
                ...
                <b-col nopadding cols="1">
                  <span style="position: absolute; top: 0px">
                    <b-button
                      ...
                      variant="link"
                      title="Remove row"
                      @click="delete_terminology_row(index)"
                    >
                      <b-icon
                        font-scale=".9"
                        icon="x-circle"
                  ...
                  <span style="position: absolute; bottom: 0px">
                    <b-button
                      ...
                      variant="link"
                      title="Add row"
                      @click="add_terminology_row(index)"
                    >
                      <b-icon
                        font-scale=".9"
                        icon="plus-square"
            ...
            <div v-else>
              ...
              <div v-if="index < customTerminologyUnsaved.length">
                ...
                <b-form-input
                  ...
                  placeholder="(required)"
                  :state="
                    item[key] !== '' && item[key] !== undefined ? null : false
                  "
                />
              </div>
              <div v-else>
  ...
  -->
              <b-button
                v-if="customTerminologySelected !== ''"
              ...
              <b-button
                ...
                >Remove Language</b-button
              >
            </span>
          </template>
        </b-table>
        <div v-if="validTerminologyName === false" style="color: red">
          Invalid terminology name. Valid characters are a-z, A-Z, and 0-9. Max
          length is 200.
        </div>
        <div
          v-else-if="
      ...
      >
        <p>
          Saving will overwrite the existing {{ selected_lang }} translation.
          Are you sure?
        </p>
      </b-modal>
      <b-modal
        ...
        title="Add Language"
        ok-title="Save"
        :ok-disabled="newLanguageCode === ''"
        @ok="add_language_request()"
      >
      ...
      <b-modal
        ref="remove-language-modal"
        title="Remove Language"
        ok-title="Remove"
        :ok-disabled="removeLanguageCode === ''"
        @ok="remove_language_request()"
      >
...
export default {
  ...
  computed: {
    customTerminologyFields: function () {
      return [this.sourceLanguageCode].concat(
        this.alphabetized_language_collection
          .map((x) => x.value)
          .filter((y) => this.removedLanguages.includes(y) === false)
      );
    },
    customTerminologyLastTableField: function () {
      ...
      if (this.alphabetized_language_collection.length > 0) {
        return this.alphabetized_language_collection[
          this.alphabetized_language_collection.length - 1
        ].value;
      } else {
        return "";
      }
    ...
    validCSV: function () {
      for (let key in this.customTerminologyUnion) {
        for (let term in this.customTerminologyUnion[key]) {
          if (
            this.customTerminologyUnion[key][term] === null ||
            this.customTerminologyUnion[key][term] === ""
          )
            return false;
        }
      }
    ...
    validTerminologyName: function () {
      ...
      if (
        ...
        (this.customTerminologyCreateNew.match(letterNumber) &&
          this.customTerminologyCreateNew.length < 256)
      ) {
        return true;
      } else {
        return false;
    ...
    alphabetized_language_collection: function () {
      ...
      // is in this.translations.Collection, and sort by the languages in that union.
        ...
        custom_terminology_union.forEach((terminology_row) => {
          for (const language_code in terminology_row) {
            const language_label = this.translateLanguages.filter(
              (x) => x.value === language_code
            )[0].text;
            // avoid adding duplicates
            if (
              translations_collection.filter(
                (x) => x["text"] === language_label
              ).length === 0
            )
              translations_collection.push({
                text: language_label,
                value: language_code,
              });
          }
      ...
      translations_collection = translations_collection.filter(
        (x) => this.removedLanguages.includes(x.value) === false
      );
      return translations_collection
    ...
    vtt_url: function () {
      if (this.selected_lang_code !== "") {
        let vttcaption = this.vttcaptions.filter(
          (x) => x.lang === this.selected_lang_code
        )[0];
        if (vttcaption) {
    ...
    srt_url: function () {
      if (this.selected_lang_code !== "") {
        let srtcaption = this.srtcaptions.filter(
          (x) => x.lang === this.selected_lang_code
        )[0];
        if (srtcaption) {
    ...
    pollyaudio_url: function () {
      if (this.selected_lang_code !== "") {
        let pollyaudiotranscript = this.pollyaudiotranscripts.filter(
          (x) => x.lang === this.selected_lang_code
        )[0];
    ...
    text_direction: function () {
      ...
      if (
        this.selected_lang_code === "ar" ||
        this.selected_lang_code === "fa" ||
        this.selected_lang_code === "he" ||
        this.selected_lang_code === "ur"
      )
        return "rtl";
      else return "ltr";
    },
  ...
  methods: {
    ...
    listTerminologiesRequest: async function () {
      ...
      try {
        ...
        this.customTerminologyList = response.data[
          "TerminologyPropertiesList"
        ].map((x) => x.Name);
      } catch (error) {
        console.log("ERROR: Failed to get vocabularies.");
        console.log(error);
      }
    },
    getLanguageList: async function () {
      ...
      try {
        ...
        response.data.results.CaptionsCollection.forEach((item) => {
          let languageLabel = vm.translateLanguages.filter(
            (x) => x.value === item.TargetLanguageCode
          )[0].text;
          // save the language code to the translationsCollection
          this.translationsCollection.push({
            text: languageLabel,
            value: item.TargetLanguageCode,
          });
        });
        ...
        if (this.alphabetized_language_collection.length > 0) {
          ...
          this.selected_lang_code =
            this.alphabetized_language_collection[0].value;
        ...
        console.log("ERROR: Failed to get languages.");
        console.log(error);
    ...
    getVttCaptions: async function () {
      ...
      try {
        ...
        this.asyncForEach(
          response.data.results.CaptionsCollection,
          async (item) => {
            ...
            if (this.showVideo === "show-video") {
              ...
              if (vm.selected_lang_code !== "") {
                ...
                let old_track = vm.player
                  .textTracks()
                  ["tracks_"].filter(
                    (x) => x.language == vm.selected_lang_code
                  )[0];
    ...
    downloadAudioFile() {
      const blob = new Blob([this.pollyaudio_url], {
        type: "audio/mpeg",
        autoplay: "0",
        autostart: "false",
        endings: "native",
          ...
          if (
            ...
            if (old_phrase != "" && new_phrase != "") {
              ...
              this.customTerminologyUnsaved =
                this.customTerminologyUnsaved.filter((item) => {
                  return item.original_phrase !== old_phrase;
                });
              // add old_phrase to custom terminology
              this.customTerminologyUnsaved.push({
                [this.sourceLanguageCode]: "",
                [this.selected_lang_code]: new_phrase,
              });
              console.log(
                "CUSTOM TERMINOLOGY: " +
                  JSON.stringify(this.customTerminologyUnsaved)
              );
            }
            old_phrase = "";
            new_phrase = "";
    ...
    getWorkflowConfig: async function () {
      ...
      try {
        ...
        this.terminology_used =
          response.data.Configuration.Translate.TranslateWebCaptions.TerminologyNames.map(
            (x) => x.Name
          );
        this.parallel_data_used =
          response.data.Configuration.Translate.TranslateWebCaptions.ParallelDataNames.map(
            (x) => x.Name
          );
        this.workflow_definition = response.data.Workflow;
        const operator_info = [];
        const sourceLanguage = this.translateLanguages.filter(
          (x) => x.value === this.sourceLanguageCode
        )[0].text;
        ...
        if (this.terminology_used) {
          if (this.terminology_used.length === 1)
            operator_info.push({
              ...
              value: this.terminology_used[0],
            });
          else
            operator_info.push({
              name: "Custom Terminologies",
              value: this.terminology_used.join().replace(/,/g, ", "),
            });
        }
        if (this.parallel_data_used) {
          if (this.parallel_data_used.length === 1)
            operator_info.push({
              name: "Parallel Data",
              value: this.parallel_data_used[0],
            });
          else
            operator_info.push({
              name: "Parallel Data",
              value: this.parallel_data_used.join().replace(/,/g, ", "),
            });
        }
    ...
    rerunWorkflow: async function () {
      ...
      data["Configuration"]["Translate"]["TranslateWebCaptions"].MediaType =
        "MetadataOnly";
      let apiName = "mieWorkflowApi";
      ...
      try {
        ...
        console.log(
          "Media assigned asset id and workflow id: " +
            asset_id +
            "workflow id:" +
            wf_id
        );
    ...
    saveTerminologyRequest: async function () {
      ...
      let requestOpts = {
        ...
        response: true,
        body: {
          terminology_name: this.customTerminologyName,
          terminology_csv: csv,
        },
        queryStringParameters: {}, // optional
      };
      ...
      try {
        ...
        if (response.status === 200) {
          ...
          this.terminologyNotificationMessage =
            "Saved terminology: " + this.customTerminologyName;
          ...
          this.vocabularyNotificationMessage =
            "Failed to save terminology: " + this.customTerminologyName;
        ...
        alert("ERROR: Failed to save terminology.");
        console.log(error);
      }
    },
    ...
    deleteTerminologyRequest: async function (customTerminologySelected) {
      if (customTerminologySelected.length === 1)
        this.terminologyNotificationMessage =
          "Deleted terminology: " + customTerminologySelected;
      if (customTerminologySelected.length > 1)
        this.terminologyNotificationMessage =
          "Deleted terminologies: " + customTerminologySelected;
      ...
      for (const i in customTerminologySelected) {
        ...
        try {
          ...
          if (response.status === 200) {
            ...
            this.terminologyNotificationMessage =
              "Failed to delete vocabulary: " + customTerminologySelected[i];
            this.terminologyNotificationStatus = "danger";
            this.showTerminologyNotification = 5;
          }
          // clear the custom terminology name used in the save terminology modal form
          ...
          alert("ERROR: Failed to delete terminology.");
          console.log(error);
        }
      }
    },
    downloadTerminology: async function () {
      ...
      for (const i in this.customTerminologySelected) {
        ...
        try {
          ...
          this.customTerminologySaved =
            this.customTerminologySaved.concat(json);
        } catch (error) {
          alert("ERROR: Failed to delete terminology.");
          console.log(error);
        }
      ...
      if (this.showVideo === "show-video") {
        ...
        if (this.player.textTracks().length > 0) {
          this.player
            .textTracks()
            ["tracks_"].filter(
              (x) => x.language == this.selected_lang_code
            )[0].mode = "showing";
        }
    ...
    getWebCaptionPages: async function (asset_id, operator_name, cursor) {
      ...
      try {
        ...
        if (response.data.results) {
          ...
          this.isBusy = false;
          if (cursor) this.getWebCaptionPages(token, url, cursor);
        } else {
    ...
    add_language_request() {
      ...
      this.removedLanguages = this.removedLanguages.filter(
        (x) => x !== this.newLanguageCode
      );
      // add the new language as a new column in the terminology table
      const language_label = this.translateLanguages.filter(
        (x) => x.value === this.newLanguageCode
      )[0].value;
      this.addedLanguages = this.addedLanguages.concat({
        text: language_label,
        value: this.newLanguageCode,
      });
      ...
      this.customTerminologyUnsaved =
        this.customTerminologyUnsaved.concat(terminology_row);
    ...
    remove_language_request() {
      ...
      this.addedLanguages = this.addedLanguages.filter(
        (x) => x.value !== this.removeLanguageCode
      );
      this.removedLanguages = [this.removeLanguageCode].concat(
      ...
      this.customTerminologyUnsaved.map(
        (x) => delete x[this.removeLanguageCode]
      );
    ...
    add_terminology_row(index) {
      ...
      if (index < this.customTerminologyUnsaved.length) {
        this.customTerminologyUnsaved.splice(
          index + 1,
          0,
          this.getEmptyTerminologyRecord()
        );
      } else {
        this.customTerminologyUnsaved.splice(
          index - this.customTerminologyUnsaved.length,
          0,
          this.getEmptyTerminologyRecord()
        );
      }
    ...
    delete_terminology_row(index) {
      ...
      if (index < this.customTerminologyUnsaved.length) {
        ...
        const index_into_saved_terminology =
          index - this.customTerminologyUnsaved.length;
